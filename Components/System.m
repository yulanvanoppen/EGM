...
properties (Access = private)
        ...
        MEX                                                                 % MEX model name
        MEXf                                                                % MEX function
        AUXMEX                                                              % equivalent for sensitivity models
        AUXMEXf
    ...
    methods
        function obj = System(model_file, auxiliary_file, varargin)     % Constructor
            parser = inputParser;
            parser.KeepUnmatched = true;                                         
            addRequired(parser, 'model_file', @(x) (ischar(x) || isstring(x)) ...
                                                   && endsWith(x, '.txt') );
            addRequired(parser, 'auxiliary_file', @(x) (ischar(x) || isstring(x)) ...
                                                   && endsWith(x, '.txt') );
            addParameter(parser, 'FixedParameters', string([]), @isstring); 
            addParameter(parser, 'FixedValues', [], @isnumeric);
            parse(parser, model_file, auxiliary_file, varargin{:});
            ...
            obj.process(model_file, auxiliary_file);                        % extract relevant functions using IQMmodel
        end
        ...
        function traces = auxiliary(obj, parameters, init, t, tol)      % Numerically integrate auxiliary ODE system
            if nargin < 5, tol = 1e-6; end
            options = struct('reltol', tol);
            ...
            if size(init, 1) == 1
                init = repmat(init, nseries, 1);                            % replicate if necessary
            end
            init_Phi = reshape(eye(obj.K), 1, []);
            init = [init repmat(init_Phi, nseries, 1)];
            
            full_parameters = zeros(1, length(obj.parameters));             % substitute fixed parameter values
            full_parameters(obj.fixed_indices) = obj.fixed.values;
            
            traces = zeros(length(t), obj.K*(obj.K+1), nseries);
            for cell = 1:nseries                                            % substitute cell-specific parameters
                full_parameters(obj.variable_indices) = parameters(cell, :);
                out = obj.AUXMEXf(t, init(cell, :), ...                     % fast integration using MEXmodel
                                  full_parameters, options);
                traces(:, :, cell) = out.statevalues;                       % save integrated states
            end
        end
        ...
        function process(obj, model_file, auxiliary_file)               % Process model files
            ...
            auxiliary_file = char(auxiliary_file);
            installIQMtools;
            auxiliary_model = IQMmodel(auxiliary_file);                     % read model file
            obj.AUXMEX = ['AUXMEX' replace(model_file, {'model_', '.txt'}, {'_', ''})];
            IQMmakeMEXmodel(auxiliary_model, obj.AUXMEX);                   % convert to MEXmodel
            obj.AUXMEXf = str2func(obj.AUXMEX);
        end
...
% function traces = auxiliary2(obj, parameters, init, t, tol)     % Numerically integrate auxiliary ODE system
%     if nargin < 5, tol = 1e-6; end
%     options = struct('reltol', tol);
% 
%     nseries = size(parameters, 1);                                  % number of replicates to integrate
...
%     full_parameters(obj.fixed_indices) = obj.fixed.values;
% 
%     init = [init zeros(size(init))];
%     for cell = 1:nseries
%         init(cell, end/2+1:end) = obj.MEXf(0, init(cell, 1:end/2), ...
%                                            full_parameters, struct('xdotcalc', 1));
%     end
% 
%     init_Phi = reshape(eye(2*obj.K), 1, []);
%     init = [init repmat(init_Phi, nseries, 1)];
% 
%     traces = zeros(length(t), 2*obj.K*(2*obj.K+1), nseries);
%     for cell = 1:nseries                                            % substitute cell-specific parameters
%         full_parameters(obj.variable_indices) = parameters(cell, :);
%         out = obj.AUXMEXf(t, init(cell, :), ...                     % fast integration using MEXmodel
%                           full_parameters, options);
