classdef Smoother < handle
    properties (SetAccess = private)
        ...
        scaled_gm                                                           % first stage scaled time grid 
        ...
        variances_gm                                                        % and first stage time grid
        lincomb_data
    end
    
    
    methods
        function obj = Smoother(system, data, settings)                 % Constructor
            ...
            obj.scaled_gm = (obj.settings.t_gm - obj.data.t(1)) / range(obj.data.t);
            ...
            obj.variances_gm = zeros(length(settings.t_gm), obj.L, obj.N);
            obj.lincomb_data = cell(obj.L, obj.N);
        end
        ...
        function update_knots(obj, knots, states)
            ...
            [obj.B, obj.dB, obj.B_fine, obj.dB_fine, ...
                    obj.data.basis_gm, obj.data.dbasis_gm] = deal(cell(1, obj.L));
            
            for k = 1:obj.L
                obj.B{k} = obj.bsplines{k}.basis(obj.scaled)';              % B-spline bases evaluated at t
                obj.dB{k} = obj.bsplines{k}.basis_diff(obj.scaled)' / range(obj.data.t);
                obj.data.basis_gm{k} = obj.bsplines{k}.basis(obj.scaled_gm)';   % save for inference
                obj.data.dbasis_gm{k} = obj.bsplines{k}.basis_diff(obj.scaled_gm)' / range(obj.data.t); 
                obj.B_fine{k} = obj.bsplines{k}.basis(obj.scaled_fine)';    % save for smooth plotting
                obj.dB_fine{k} = obj.bsplines{k}.basis_diff(obj.scaled_fine)' / range(obj.data.t);
            end
        ...
        function optimize(obj, states)
            ...
            obj.data.variances = obj.variances_gm;                          % measurement and first-stage time points
        ...
        function update_coefficients(obj, states)                       % Spline coefficients from current variances
            for i = 1:obj.N                                                 % W: correcting weights from variances
                for k = states                                              % penalty: lambda * L2(spline basis/coefficients)
                    ...
                    obj.lincomb_data{k, i} = obj.B{k}' * W * obj.B{k} \ (obj.B{k}' * W);
                    obj.delta{k}(:, i) = obj.lincomb_data{k, i} * obj.data.traces(:, k, i);
                end
            end
        end
        ...
        function update_variances(obj, states)                          % Estimate variances from current splines
            for k = states                                                  % smoothed measurements
                ...
                predicted_gm = obj.data.basis_gm{k} * reshape(obj.delta{k}, obj.bsplines{k}.card, obj.N);
                design_gm = [ones(obj.N * obj.T_gm, 1) flatten(predicted_gm).^2];
                obj.variances_gm(:, k, :) = reshape(design_gm * coefficients', obj.T_gm, 1, obj.N);
            end
        end
        
        
        function fit(obj)                                               % Fitted splines from estimated coefficients
            obj.data.smoothed = zeros(obj.T_gm, obj.L, obj.N);              % smoothed measurements
            obj.data.dsmoothed = zeros(obj.T_gm, obj.L, obj.N);             % corresponding gradients
            ...
            for i = 1:obj.N
                for k = 1:obj.L                                             % multiply coefficients with bases
                    obj.data.smoothed(:, k, i) = obj.data.basis_gm{k} * obj.delta{k}(:, i);
                    obj.data.dsmoothed(:, k, i) = obj.data.dbasis_gm{k} * obj.delta{k}(:, i);
                    obj.data.smoothed_fine(:, k, i) = obj.B_fine{k} * obj.delta{k}(:, i);
                    obj.data.dsmoothed_fine(:, k, i) = obj.dB_fine{k} * obj.delta{k}(:, i);
                end
            end
            ...
            obj.data.var_delta = cell(obj.N, obj.L);
            for i = 1:obj.N
                for k = 1:1:obj.L
                    S = diag(max(reshape(obj.data.variances_sm(:, k, i), 1, []), 1e-10));
                    obj.data.var_delta{i, k} = tryinv(obj.B{k}' * (S \ obj.B{k}));
                end
            end

            obj.data.lincomb_data = obj.lincomb_data;
        end
